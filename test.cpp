...
int main() {
  constexpr float TEST_FLOATS[] = { 0.1f, 0.5f, 1.75f, 10.10f, 0.001f, 321325432.321f, -543254325.5432f, 0.0000000413f, 1e-20, std::numeric_limits<float>::infinity(), -std::numeric_limits<float>::infinity() /*, std::numeric_limits<float>::max() */, std::numeric_limits<float>::quiet_NaN(), /* std::numeric_limits<float>::min(), */ -0.0f, 0.0f};  
  ...
  for (const float f : TEST_FLOATS) {
    ...
    ASSERT_ALMOST_EQ(Float32{std::bit_cast<uint32_t>(std::sqrt(f))}, sqrt(fa), "Wrong result of sqrt(" << f << ")");
  ...
  */
  ...
  std::cout << "🎉 All tests succeded! 🎉" << std::endl;
}
